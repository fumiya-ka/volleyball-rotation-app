    ...
    <div className="space-y-4 p-4">
      ...
      <div className="flex gap-3 flex-wrap">
        <select
          ...
          className="font-mono text-[15px] bg-[#0d1424] border border-[#2d3340] text-[#c9cdd4] px-3 py-2 focus:border-[#ff5436] focus:outline-none"
        >
      ...
      {phase === 'serve' && (
        <div className="space-y-2.5">
          {(['server', 'frontRow', 'backRow'] as const).map((field) => (
            <div key={field}>
              <label className="font-mono text-[14px] text-[#6b7280] block mb-1">
              ...
              <input
                ...
                className="w-full font-mono text-[15px] bg-[#0d1424] border border-[#2d3340] text-[#c9cdd4] px-3 py-2 focus:border-[#ff5436] focus:outline-none"
              />
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={5}
          className="w-full font-mono text-[15px] bg-[#0d1424] border border-[#2d3340] text-[#c9cdd4] px-3 py-2.5 focus:border-[#ff5436] focus:outline-none resize-y leading-relaxed"
      ...
      {/* 攻撃テンポ解説 */}
      {phase === 'attack' && (
        <div className="border-t border-[#1e2840] pt-4 space-y-3">
          <p className="font-mono text-[14px] text-[#6b7280]">攻撃テンポ解説（全ローテーション共通）</p>
          {(['1st', '2nd', '3rd'] as const).map((tempo) => (
            <div key={tempo} className="space-y-1.5">
              <p className="font-mono text-[14px] text-[#ff5436]">{tempo} テンポ</p>
              <textarea
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      {desc ? (
        ...
        <p className="font-mono text-[15px] text-[#4b5563]">データが見つかりません</p>
      )}
...
export default function DescriptionsEditor() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {isDirty && (
        <div className="px-4 py-2 bg-[#1a2035] border-b border-[#1e2840]">
          <span className="font-mono text-[14px] text-[#fbbf24]">● 未保存の変更あり — Download で JSON を書き出してください</span>
        </div>
      )}
